...
export ROFI_THEME_MAIN="window { width: ${W_MAIN}px; border: ${ROFI_BORDER:-2}px solid; border-radius: ${ROFI_RADIUS:-16}px; border-color: $SEL; background-color: $BG; } mainbox { children: [ inputbar, listview ]; padding: 20px; } inputbar { margin: 0px 0px 15px 0px; padding: 10px; background-color: $BGA; border-radius: 12px; children: [ prompt, entry ]; } prompt { text-color: $SEL; font: \"JetBrainsMono Nerd Font Mono Bold ${ROFI_FONT_SIZE:-12}\"; } entry { text-color: $FG; font: \"JetBrainsMono Nerd Font Mono ${ROFI_FONT_SIZE:-12}\"; } listview { columns: 1; lines: 9; spacing: 8px; dynamic: true; } element { padding: 12px 15px; border-radius: 12px; text-color: $FG; } element selected { background-color: $SEL; text-color: $BG; } element-text { vertical-align: 0.5; font: \"JetBrainsMono Nerd Font Mono ${ROFI_FONT_SIZE:-12}\"; }"
